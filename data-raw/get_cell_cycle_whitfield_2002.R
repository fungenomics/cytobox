...
# Data obtained from Whitfield et al, 2002
library(data.table)
library(dplyr)
library(usethis)
...
cell.cycle.genes.whitfield.2002 <- fread("/gs/project/txk-653-aa/blancha/analyses/njabado/single_cell/samples/ET_CT_12/data/CellCycleGeneList_1134_whitfield_2002_mice_gene_symbols.txt", data.table=FALSE)

colnames(cell.cycle.genes.whitfield.2002) <- tolower(colnames(cell.cycle.genes.whitfield.2002))
colnames(cell.cycle.genes.whitfield.2002)[1] <- "mmusculus.gene.symbol"

usethis::use_data(cell.cycle.genes.whitfield.2002, overwrite=TRUE)
