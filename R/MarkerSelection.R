...
########################################################3
  ...
  # append markers
  retFrame<-data.frame(ordered_marker.types,gene_id,filtered_errors)
  ...
  colnames(retFrame)[(NCOL(gene_id)*2+2)] <- "false negative"
  print("false negative named")
  colnames(retFrame)[(NCOL(gene_id)*2+1)] <-"false positive"
